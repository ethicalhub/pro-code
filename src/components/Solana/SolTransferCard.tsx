...
export function SolTransferCard() {
  ...
  async function sendSol() {
    ...
    setError(null);
    try {
      console.log(`Sending ${lamports} lamports to ${dest}...`);
      await send({
      ...
      if (err != null && typeof err === "object" && "transactionPlanResult" in err) {
        const result = (err as { transactionPlanResult: unknown }).transactionPlanResult;
        const detail =
          result != null && typeof result === "object" && "cause" in result
            ? String((result as { cause: unknown }).cause)
